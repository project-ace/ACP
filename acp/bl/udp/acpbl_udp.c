...
static int iacp_init(void)
{
  ...
  debug printf("rank %d - rank_table 0x%016llx\n", MY_RANK, RANK_TABLE);
  debug printf("rank %d - port_table 0x%016llx\n", MY_RANK, PORT_TABLE);
  debug printf("rank %d - addr_table 0x%016llx\n", MY_RANK, ADDR_TABLE);
...
#ifdef DEBUG
  printf("end finalize gma\n");
  fflush(stdout);
#endif
  
  iacpbludp_finalize_gmm();
#ifdef DEBUG
  printf("end finalize gmm\n");
  fflush(stdout);
#endif
...
int acp_finalize(void)
{
  ...
  r = iacp_finalize();
#ifdef DEBUG
  printf("end iacp_finalize\n");
  fflush(stdout);
#endif
  
  free(RANK_TABLE);
#ifdef DEBUG
  printf("end free rank table\n");
  fflush(stdout);
#endif
  free(PORT_TABLE);
#ifdef DEBUG
  printf("end free port table\n");
  fflush(stdout);
#endif
  free(ADDR_TABLE);
#ifdef DEBUG
  printf("end free addr table\n");
  fflush(stdout);
#endif
  RANK_TABLE = NULL;
  PORT_TABLE = NULL;
...
/* int acp_reset(int rank, size_t size) */
