...
acp_list_t acp_collapse_set(acp_set_t set)
{
    ...
    uint64_t size_directory = set.num_ranks * 8;
    uint64_t size_lock_var  = 8;
    uint64_t size_list_tmp  = 24;
    uint64_t size_list_new  = 24;
    uint64_t offset_directory = 0;
    uint64_t offset_lock_var  = offset_directory + size_directory;
    uint64_t offset_list_tmp  = offset_lock_var  + size_lock_var;
    uint64_t offset_list_new  = offset_list_tmp  + size_list_tmp;
    ...
    acp_ga_t buf_list_new  = buf + offset_list_new;
    void* ptr = acp_query_address(buf);
    volatile acp_ga_t* directory = (volatile acp_ga_t*)(ptr + offset_directory);
    volatile uint64_t* lock_var  = (volatile uint64_t*)(ptr + offset_lock_var);
    volatile uint64_t* list_tmp  = (volatile uint64_t*)(ptr + offset_list_tmp);
    volatile uint64_t* list_new  = (volatile uint64_t*)(ptr + offset_list_new);
    ...
    list_new[0] = ACP_GA_NULL;
    list_new[1] = ACP_GA_NULL;
    list_new[2] = 0;
    ...
    for (rank = 0; rank < set.num_ranks; rank++) {
        ...
        for (slot = 0; slot < set.num_slots; slot++) {
            acp_ga_t ga_lock_var = ga_table + slot * 32;
            acp_ga_t ga_list_tmp = ga_lock_var + 8;
            do {
                acp_cas8(buf_lock_var, ga_lock_var, 0, 1, ACP_HANDLE_NULL);
                acp_copy(buf_list_tmp, ga_list_tmp, size_list_tmp, ACP_HANDLE_ALL);
                acp_complete(ACP_HANDLE_ALL);
            } while (*lock_var != 0);
            if (list_tmp[2] > 0) {
                if (list_new[0] != ACP_GA_NULL) {
                    acp_copy(list_tmp[0] + 8, buf_list_new + 8, 8, ACP_HANDLE_ALL);
                    acp_copy(list_new[1]    , buf_list_tmp    , 8, ACP_HANDLE_ALL);
                    acp_complete(ACP_HANDLE_ALL);
                } else
                    list_new[0] = list_tmp[0];
                list_new[1] = list_tmp[1];
                list_new[2] += list_tmp[2];
            }
        }
        acp_free(ga_table);
    }
    ...
    if (list_new[0] != ACP_GA_NULL) {
        list_tmp[0] = list.ga;
        acp_copy(list_new[0] + 8, buf_list_tmp, 8, ACP_HANDLE_ALL);
        acp_copy(list_new[1]    , buf_list_tmp, 8, ACP_HANDLE_ALL);
    } else {
        list_new[0] = list.ga;
        list_new[1] = list.ga;
    }
    acp_copy(list.ga, buf_list_new, size_list_new, ACP_HANDLE_ALL);
    acp_complete(ACP_HANDLE_ALL);
