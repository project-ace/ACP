...
static int *rrm_get_flag_tb; /* rrm get flag tb */
static int *rrm_reset_flag_tb; /* rrm reset flag tb */
...
acp_atkey_t acp_register_memory(void* addr, size_t size, int color){
  ...
  if(found == FAULSE){
	...
	handle = acp_copy(dst + base_offset + sizeof(int) * (acp_numprocs  + myrank),
			  src + base_offset + sizeof(int) * i, 
			  sizeof(int), ACP_HANDLE_NULL);
      }
    }
	...
	fprintf(stdout, "%d ", rrm_ack_flag_tb[i]);
      ...
      fprintf(stdout, "\n");
#endif
      if(0 == memcmp(rrm_get_flag_tb, rrm_ack_flag_tb, sizeof(int) * nprocs)){
	break;
      }
    }
    /* initialization rrm flag table */
    memset(rrm_get_flag_tb, 0, sizeof(int) * nprocs);
    memset(rrm_ack_flag_tb, 0, sizeof(int) * nprocs);
    
    /* insesrt memory region */
    for(i = 0;i < MAX_RM_SIZE;i++){
      if(lrmtb[i].size == 0){
	found = TRUE;
	if(lrmtb[i].lock == FAULSE){
...
int putrrmgetflag(acp_handle_t handle, int torank){  
  ...
  sr.wr.rdma.remote_addr = smi_tb[torank].addr + 
    ...
    sizeof(CMD) * MAX_RCMDB_SIZE + sizeof(CMD) * 2  + sizeof(int) * myrank;
  sr.wr.rdma.rkey = smi_tb[torank].rkey;
...
int putrrmackflag(acp_handle_t handle, int torank){  
  ...
  sr.wr.rdma.remote_addr = smi_tb[torank].addr + 
    acp_smsize + sizeof(RM) * MAX_RM_SIZE * 2 + sizeof(uint64_t) * 5 + sizeof(CMD) * MAX_CMDQ_ENTRY + 
    sizeof(CMD) * MAX_RCMDB_SIZE + sizeof(CMD) * 2  + sizeof(int) * acp_numprocs * 2 + sizeof(int) * myrank;
  sr.wr.rdma.rkey = smi_tb[torank].rkey;
	...
	if((wc.wr_id & MASK_WRID_RCMDB) == 0){
	  ...
	  while(index < *rcmdbuf_tail ){
		...
		if(rcmdbuf[idx].type == COPY){
		  /* get rank and tag, offset of src ga */
...
int iacp_init(void){
  ...
  *rcmdbuf_tail = 0;
  ...
  rrm_get_flag_tb = (int *)((char*)putcmdbuf + sizeof(CMD));

  /* initilaize reset flag table */
  rrm_reset_flag_tb = (int *)((char*)rrm_get_flag_tb + sizeof(int) * acp_numprocs);
  
  /* initialize ack flag table */
  rrm_ack_flag_tb = (int *)((char*)rrm_reset_flag_tb + sizeof(int) * acp_numprocs);
