...
static inline int cs_add(uint64_t cqp, uint64_t time)
{
  ...
  if (cstail >= 0)
    ...
    cshead = pos;
  cstail = pos;
  debug printf("rank %d - cs_add  cqp %6d time %llu pos %d\n", MY_RANK, cqp, time, pos);
...
{
  debug printf("rank %d - cs_free cqp %6d time %llu pos %d count %d\n", MY_RANK, cs[pos].cqp, time, pos, cs[pos].count);
  if (cs[pos].prev >= 0)
  ...
  while (!abort_comm_thread) {
    ...
    for (i = 0; i < cqnum; i++) {
      ...
      if (cq[p].stat == CQSTAT_COPY0) {
        debug printf("rank %d - memcpy  cqp %6d time %llu dst 0x%016llx src 0x%016llx size %llu\n", MY_RANK, cqp, time, ga2address(cq[p].dst), ga2address(cq[p].src), cq[p].size);
      ...
      if (cq[p].stat == CQSTAT_ATOMIC0) {
        ...
        debug printf("rank %d - local atomic cqp %6d time %llu dst 0x%016llx src 0x%016llx type %u result %10u new_value %10u\n", MY_RANK, cqp, time, ga2address(cq[p].dst), ga2address(cq[p].src), cq[p].type, *(uint32_t*)ga2address(cq[p].dst), *(uint32_t*)ga2address(cq[p].src));
        sync_synchronize();
        cq[p].stat = 0;
      }
    }
          ...
          for (i = cshead; i >= 0; i = cs[i].next)
            if (cs[i].rank == rank)
              if (compare_seq(*dg_seq(cs[i].dg), seq) > 0) {
                if      (cq[cs[i].cqp].stat == CQSTAT_COPY3TRY   ) cq[cs[i].cqp].stat = CQSTAT_COPY3WAIT;
                else if (cq[cs[i].cqp].stat == CQSTAT_COPY1TRY   ) cq[cs[i].cqp].stat = 0;
                else if (cq[cs[i].cqp].stat == CQSTAT_ATOMIC2TRY ) cq[cs[i].cqp].stat = CQSTAT_ATOMIC2WAIT;
                else if (cq[cs[i].cqp].stat == CQSTAT_ATOMIC1TRY ) cq[cs[i].cqp].stat = 0;
                cs_free(i, time);
    ...
    for (i = dshead; i >= 0; i = ds[i].next) {
      if (time >= ds[i].time) {
        if (ds[i].stat == DSSTAT_COPY_READ || ds[i].stat == DSSTAT_COPY2_READ) {
          ...
          ds[i].time = time + ((uint64_t)(0x4000 + (i << (12 - BIT_DS))) << ((ds[i].count < 7) ? ds[i].count + 4 : 7 + 4));
          ds[i].count++;
        }
      }
    }
      ...
      for (j = ds[i].swhead; j >= 0; j = ds[i].sw[j].next) {
        ...
        if (time >= ds[i].sw[j].time) {
          ...
          *dg_dst(ds[i].dg) = ds[i].sw[j].src;
          ...
          /* Set retransmit timer from 200K to 200M clock (around 100 usec to 100 msec) */
          ds[i].sw[j].time = time + ((uint64_t)(0x4000 + (i << (12 - BIT_DS))) << ((ds[i].sw[j].count < 10) ? ds[i].sw[j].count + 4 : 10 + 4));
          ds[i].sw[j].count++;
        }
      }
    }
...
int iacpbludp_finalize_gma(void)
{
  ...
  sync_synchronize();
#ifdef DEBUG
  printf("end sync_synchronize\n");
  fflush(stdout);
#endif
  pthread_join(comm_thread_id, NULL);
#ifdef DEBUG
  printf("end pthread_join\n");
  fflush(stdout);
#endif
