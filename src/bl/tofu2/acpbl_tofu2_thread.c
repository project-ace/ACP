...
void command_station(cq_t *command)
{
  int rc;
...
static inline void tofu_receive_delegation(tofu_trans_stat_t *tofu_trans_stat)
{
  ...
  dprintf("tofu_receive_delegation@rank%d: id=%d\n", myrank_sys, id);
...
static inline void tofu_transfer_end(tofu_trans_stat_t *tofu_trans_stat)
{
  ...
  dprintf("tofu_transfer_end@rank%d: id=%d\n", myrank_sys, id);
  ...
  if((id & COMM_REPLY) == 0){
	...
	dprintf("SACHIKO EX\n");
	if(command->copy.base.cmd >= CMD_CAS4){
	  dprintf("JAS\n");
	  ga = command->cas4.ga_dst;
	  dprintf("tofu_transfer_end@rank%d: id=%d, cmd=%d, ga=0x%lx, ptr=%p, data=%lu\n",
		  myrank_sys, id, command->copy.base.cmd, ga, (uint64_t *)acp_query_address(ga), tofu_trans_stat->data);//fflush(stdout);
	  if(command->cas4.base.cmd >= CMD_CAS8){
	    *((uint64_t *)acp_query_address(ga)) = tofu_trans_stat->data;
	  }else{
	    *((uint32_t *)acp_query_address(ga)) = tofu_trans_stat->data >> 32;
	  }
	}
