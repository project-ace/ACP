  ...
  for(i=0; i<num_colors; i++){
    if(color_bit & 1){
      rc = _acpblTofu_sys_register_memory(localtag_table[localtag]->addr_head,
					  ...
					  i, localtag);
      if(rc){
	//printf("_acpblTofu_enable_localtag@rank%d: rc=%d\n", myrank_sys, rc);
...
{
  /*printf("_acpblTofu_gen_atkey@rank%d: rank=%d, color=%d, localtag=%d, part0=0x%016lx, part1=0x%016lx, part2=0x%016lx\n",
         myrank_sys, rank, color, localtag,
         (uint64_t)ATKEY_GEN((uint64_t)color, ga_lsb_color, ga_mask_color),
         (uint64_t)ATKEY_GEN((uint64_t)rank, ga_lsb_rank, ga_mask_rank),
         (uint64_t)ATKEY_GEN((uint64_t)localtag, ga_lsb_localtag, ga_mask_localtag));
  fflush(stdout);
  */
  /*printf("_acpblTofu_gen_atkey@rank%d: ga_lsb_color=%d, ga_mask_color=0x%016lx, ga_lsb_rank=%d, ga_mask_rank=0x%016lx, ga_lsb_localtag=%d, ga_mask_localtag=0x%016lx\n",
         myrank_sys, ga_lsb_color, ga_mask_color, ga_lsb_rank, ga_mask_rank, ga_lsb_localtag, ga_mask_localtag);
  fflush(stdout);
  */
  acp_atkey_t rv = (ATKEY_GEN((uint64_t)color, ga_lsb_color, ga_mask_color) |
		    ATKEY_GEN((uint64_t)rank, ga_lsb_rank, ga_mask_rank)    |
		    ATKEY_GEN((uint64_t)localtag, ga_lsb_localtag, ga_mask_localtag));
  /*
  printf("_acpblTofu_gen_atkey@rank%d: rv=0x%016lx\n",
         myrank_sys, rv);
  */
