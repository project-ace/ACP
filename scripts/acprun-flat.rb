####!/usr/bin/ruby
####!/home/dahon/ruby19/bin/ruby
####!/home/dahon/ruby20/bin/ruby
####!/home/dahon/ruby21/bin/ruby
#
...
class Acprun
    attr_accessor :myname,     :starter_memsize, :nprocs,         :arr_taskid, :nodefile,
                  ...
                  :nodenames,  :nodeips,         :flg_envnetwork, :comm_args,  :comms_ssh
    # private
    ...
    def print_help( port )
        ...
        port.printf( "    : --acp-envnet=ENVNETWORK                  (default: UDP. specify UDP or IB)\n" )
        port.printf( "    : -v or --acp-version\n" )
        port.printf( "    : -h or --acp-help\n" )
        port.printf( "    : other --acp-* options are judged as error.\n" )
        exit
    end
    ...
    def read_nodenames
        ...
        if @flg_localmode then
            ...
            begin
                ...
                $stderr.printf( "%s: error processing file %s\n", @nodefile )
                exit
            end
        end
        #p nodenames
    ...
    def gen_ssh_commands
        ...
        for ip in 0...@nprocs do
            comm = sprintf( "%s=%d %s=%d %s=%d %s=%d %s=%d %s=%s %s=%d %s",
                            $str_envmyrank, ip,
...
if $debug_mode then
    if $debug_level > 200 then
        ...
        for i in 0...(acprun.nprocs) do
            printf( "%20s%20s\n", acprun.nodenames[ i ], acprun.nodeips[ i ] )
        end
        for i in 0...acprun.nprocs do
